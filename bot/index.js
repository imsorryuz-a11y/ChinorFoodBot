...
const WEBAPP_URL = process.env.WEBAPP_URL
  ...
  || 'https://chinorfoodbot.onrender.com';
...
bot.start(async (ctx) => {
  ...
  await ctx.reply('👇 Mini ilovani ochish uchun bosing:', {
    reply_markup: miniAppButton(),
  });
});

// ── Asosiy klaviatura (navigatsiya uchun)
function mainKeyboard() {
  return {
    keyboard: [
      [{ text: '🛒 Savat' }, { text: '✅ Buyurtma berish' }],
      [{ text: '🍔 Ovqatlar' }, { text: '🥤 Ichimliklar' }, { text: '❓ Yordam' }]
    ],
...
function miniAppButton() {
  return {
    inline_keyboard: [[
      { text: '🚀 Menyu ochish (Mini App)', web_app: { url: WEBAPP_URL } }
    ]]
  };
}
...
// ============================================================
//  🚀  Botni ishga tushirish (auto-retry bilan)
// ============================================================
async function launchBot(retryCount = 0) {
  try {
    await bot.launch({ dropPendingUpdates: true });
    console.log('🚀 Chinor Food Bot muvaffaqiyatli ishga tushdi!');
    console.log('📡 Polling faol | Bot: @ChinorFoodBot');

    // Menu tugmasini to’g‘rilash
    try {
      ...
      console.error('Menu tugma xatolik (muhim emas):', err.message);
    }

  } catch (err) {
    if (err.message && err.message.includes('409')) {
      const delay = Math.min(30, 5 * (retryCount + 1));
      console.log(`⚠️ 409 Conflict — ${delay} sekunddan keyin qayta urinaman... (${retryCount + 1}-urinish)`);
      setTimeout(() => launchBot(retryCount + 1), delay * 1000);
    } else {
      console.error('❌ Bot launch xatosi:', err.message);
      // 10 sekunddan keyin qayta urinish
      setTimeout(() => launchBot(retryCount + 1), 10000);
    }
  }
}
